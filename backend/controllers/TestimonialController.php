    ...
    public function actionAjaxupdate() {
        ...
        if ($act == 'doDelete') {
            foreach ($_POST['autoId'] as $value) {
                $delTestimonial = Testimonial::model()->findByPk($value); // assuming there is a post whose ID is 10
                $delTestimonial->delete(); // delete the row from the database table
            }
            exit;
        }
        //testimonialAccept
        if ($act == 'testimonialAccept') {
            foreach ($_POST['autoId'] as $value) {
                $model = Testimonial::model()->findByPk($value);

...
//      $data = $model->getCatName($id);  
//      $model->parent = $id;
//      $model->cat_type = $data['cat_type'];
//
//echo $form->textField($model,'cat_name' ,array($model->cat_name,'readonly'=>true));
    // echo "<pre>";print_r($subcategory);
    // echo $subcategory[0]->cat_type;
// exit;
    ?>


    <h3>Create Sub-category of <?php echo $subcategory[0]->cat_name; ?></h3>
    <div class="row">
        <?php echo $form->labelEx($model, 'cat_type'); ?>
        <?php //echo $form->textField($model,'cat_type',$data['cat_type']);  ?>
        <?php echo $form->textField($model, 'cat_type', array('value' => $subcategory[0]->cat_type, 'readonly' => true)); ?>

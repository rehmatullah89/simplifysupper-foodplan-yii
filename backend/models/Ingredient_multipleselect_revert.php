...
class Ingredient extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('ingredient, ingweight, pantry, weight,catid', 'required'),
    ...
    public function relations() {
        ...
        return array(
            ...
            'memberingredients' => array(self::HAS_MANY, 'Memberingredient', 'ingredient_id'),
    ...
    public static function getmeasurementhere() {
        $getMeasurement = Measurement::model()->findAll();
        $measureArray = array();
        foreach ($getMeasurement as $ms)
            $measureArray[$ms->measurement] = $ms->measurement;
        if (empty($measureArray))
            array_push($measureArray, 'No Measurement Exists');
        return $measureArray;
    }
    
    public static function getmeasurementupdate($id) {
        //echo '========================='.$id;exit;
        $getMeasurement = IngredientMeasurement::model()->findAllByAttributes(array('ing_id'=>$id));
        //CVarDumper::dump($getMeasurement, 10 , TRUE);exit;
        $measureArray = array();
        foreach ($getMeasurement as $ms)
            $measureArray[$ms->measure->measurement] = $ms->measure->measurement;
        if (empty($measureArray))
            array_push($measureArray, 'No Measurement Exists');
        return $measureArray;
//        $result['test'] = 'test';
